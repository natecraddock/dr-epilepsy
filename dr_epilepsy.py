...
bl_info = {
    ...
    'version': (1, 1),
    "blender": (2, 80, 0),
    'location': '3D View > CTRL-D',
    'description': 'Dr. Epilepsy will melt your brain',
    'wiki_url': 'https://github.com/natecraddock/dr-epilepsy',
    'tracker_url': 'https://github.com/natecraddock/dr-epilepsy/issues',
    'category': '3D View'
}
...
class VIEW_3D_OT_dr_epilepsy(bpy.types.Operator):
    ...
    def modal(self, context, event):
        ...
        if event.type == 'TIMER':
            ...
            context.preferences.view.ui_scale = 1 + (random() * 3)
            
            for vb in dir(theme):
                bov = getattr(theme, vb)

                # Color buttons and other elements
...
def register():
    bpy.utils.register_class(VIEW_3D_OT_dr_epilepsy)
    kc = bpy.context.window_manager.keyconfigs.addon
    km = kc.keymaps.new(name="3D View", space_type="VIEW_3D")
    kmi = km.keymap_items.new('wm.drepilepsy', 'D', 'PRESS', ctrl=True)


def unregister():
    bpy.utils.unregister_class(VIEW_3D_OT_dr_epilepsy)
